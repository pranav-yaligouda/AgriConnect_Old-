...
exports.createRequestLimiter = rateLimit({
  ...
  message: { message: 'You have reached your daily contact request limit. Please try again tomorrow.' },
...
exports.checkRequestStatus = async (req, res) => {
  try {
    ...
    const existingRequest = await ContactRequest.findOne(query).select('_id').lean();
...
exports.createContactRequest = async (req, res) => {
  if (await hasUnresolvedAcceptedRequests(req.user._id)) {
    return res.status(429).json({ message: 'You have unresolved accepted requests older than 2 days. Complete them before making new requests.' });
  }
  try {
    ...
    if (requestedQuantity < (product.minimumOrderQuantity || 1)) {
      return res.status(400).json({ message: `Requested quantity must be at least the minimum order quantity (${product.minimumOrderQuantity || 1})` });
    }
    const farmerId = product.farmer;
    if (farmerId.equals(requesterId)) {
      return res.status(400).json({ message: 'Cannot request your own contact' });
    }
    ...
    if (dailyCount >= MAX_REQUESTS[requesterRole]) {
      return res.status(429).json({ message: `You have reached your daily contact request limit (${MAX_REQUESTS[requesterRole]} requests). Please try again tomorrow.` });
    }
    ...
    if (existingRequest) {
      return res.status(409).json({ message: 'Pending request already exists for this farmer and product', existingRequestId: existingRequest._id });
    }
    // Create new request
...
exports.acceptContactRequest = async (req, res) => {
  if (await farmerHasUnresolvedAcceptedRequests(req.user._id)) {
    return res.status(429).json({ message: 'You have unresolved accepted requests older than 2 days. Complete them before accepting new requests.' });
  }
  try {
...
exports.getMyContactRequests = async (req, res) => {
  try {
    ...
    const sent = await ContactRequest.find({ requesterId: userId })
      .populate({
        path: 'productId',
        select: 'name price unit availableQuantity minimumOrderQuantity location',
      })
      .populate({
        path: 'farmerId',
...
exports.userConfirm = async (req, res) => {
  try {
    ...
    if (!request) return res.status(404).json({ message: 'Request not found' });
    if (String(request.requesterId) !== String(userId)) return res.status(403).json({ message: 'Not authorized' });
    if (request.userConfirmed) return res.status(409).json({ message: 'Already confirmed' });
...
exports.farmerConfirm = async (req, res) => {
  try {
    ...
    const request = await ContactRequest.findById(id);
    if (!request) return res.status(404).json({ message: 'Request not found' });
    if (String(request.farmerId) !== String(farmerId)) return res.status(403).json({ message: 'Not authorized' });
    if (request.farmerConfirmed) return res.status(409).json({ message: 'Already confirmed' });
...
// Admin dispute tools
exports.getDisputes = [authorize('admin'), async (req, res) => {
  try {
...
exports.adminResolveDispute = [authorize('admin'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const request = await ContactRequest.findById(id);
    if (!request) return res.status(404).json({ message: 'Request not found' });
    if (request.status !== 'disputed') return res.status(409).json({ message: 'Not a disputed request' });
